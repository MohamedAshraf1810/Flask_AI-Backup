...
def login_user():
    if request.method == 'POST':
        ...
        if check:
                ...
                return redirect(url_for("voiceAuth"))
        else:
            print("LOGIN FAILED!!!!!!")
...
def uploadwav():
    if "username" in session:
        ...
        input_folder = Path('Voice_Recognition_Utilities/Testing_Audio/' + user_name +'/' +'oriSound.wav')
        input_folder2 = Path('Voice_Recognition_Utilities/Testing_Audio/' + user_name +'/' + user_name +'.wav')
        input_folder.parent.mkdir(exist_ok=True, parents=True)
